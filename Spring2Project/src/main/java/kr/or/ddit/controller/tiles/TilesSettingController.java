...
public class TilesSettingController {
	 ...
	 * 		1. Tiles란?
	 * 			어떤 JSP을 템플릿으로 사용하고 템플릿의 각 영역을 어떤 내용으로 채울지에 대한 정보를 설정한다.
	 * 			하나의 화면들을 만들다보면 공통적이고 반복적으로 생성해야 하는 header, footer와 같은 영역들이 존재한다.
	 * 			우리는 그러한 공통 부분들을 분리하여 반복적으로 컴포넌트들을 사용하는게 아닌 공통적인 부분은 한번만 가져다 스고
	 * 			변화하는 부분에 대해서만 동적으로 변환해 페이지를 관리할 수 있어야한다.
	 * 			이렇게 헤더/푸터/메뉴 등 공통적인 소스를 분리하여 한 화면에서 동적으로 레이아웃을 한 곳에 배치하여
	 * 			설정하고 관리할 수 있도록 도와주는 페이지 모듈화를 돕는 프레임워크입니다.
	 * 
	 * 			- 아래 jsp들을 이용하려 페이지 모듈화 진행
	 * 				template.jsp
	 * 				> header.jsp
	 * 				> content source (jsp)
	 * 				> footer.jsp
	 * 
	 * 			** 그 외에 다양한 영역의 체이지는 구현하고자 하는 시나리오를 바탕으로 페이지가 구성될 때
	 ...
	 * 				- 게시판 목록 화면 서비스 큻래스 메소드 만들기
	 * 				- 게시판 목록 화면 Mapper 인터페이스 메소드 만들기
	 * 				- 게시판 목록 화면 Mapper xml 인터페이스 메소드 만들기
	 * 				- 게시판 목록 화면 페이지에서 페이징 적용된 목록 확인
	 * 				- 여기까지 확인
	 * 				
	 * 				- 게시판 등록 컨트롤러 만들기 (crud/NoticeInsertController)
	 ...
	 * 				- 여기까지 확인
	 * 				
	 * 				- 게시판 삭제 기능 컨트롤러 메소드 만들기 (noticeUpdateForm : get)
