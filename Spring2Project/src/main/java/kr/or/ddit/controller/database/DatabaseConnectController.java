...
public class DatabaseConnectController {
	 ...
	 * 						reg_flag date default not null,
	 ...
	 * 				- query : ResultSetExtractor, RowCallbackHanbler와 함께 조회할 때 사용한다.
	 ...
	 * 				- 객체를 사용하는 시점에서 쿼리을 전달하거나 동일한 트랜젝션 안에서 처리 내용의 같음을 보장한다(패러다임의 불일치 해결)
	 * 				
	 *    			# Entity
	 ...
	 * 				Entity임을 설정하고 @Table 어노테이션으로 테이블명을 설정해주면 VO클래스는 데이터베이스의 생성되어 있는 테이블과 연결되기
	 * 				위한 준비가 된다.
	 * 
	 * 				(ex)
	 * 				@Entity
	 ...
	 * 
	 * 				JPA는 JPA 구현체인 hibernate를 이용하여 사용합니다. 그러려면 관련 의존관계를 등록 후 사용해야 합니다.
	 * 				SQL문을 이용하여 데이터베이스를 연동하여 데이터를 가공하는게 아닌, Method를 통해 데이터베이스를 조작할 수 있다는 장점이 있어
	 * 				객체 모델을 이용하여 비즈니스 로직을 구성하는데에만 집중을 할 수 있다.
	 * 
	 ...
	 * 					*** 기업 면접에서 ORM은 뭐 쓰셨어요?
	 * 						- 저는 ibatis를 사용했습니다. / 저는 mybatis를 사용했습니다
	 * 
	 * 			7. 마이바티스
